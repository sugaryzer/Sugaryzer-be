...
export class AnalysisController {
    ...
    static async get(req: UserRequest, res: Response, next: NextFunction) {
        try {
            ...
            if(req.query.daily_sugar_intake){
                const response = await AnalysisService.mlAnalysis(Number(req.query.daily_sugar_intake));
                res.status(200).json({
                    error: false,
                    message: "ML analysis retrieved successfully",
                    result: response,
            })
            } else {
                if(!req.query.date){
